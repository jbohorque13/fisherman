      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.centered}>
            <Ionicons name="notifications-off-outline" size={48} color={theme.borderInput} />
            <Text style={styles.empty}>Sin notificaciones</Text>
          </View>
        }
        renderItem={({ item }) => (
          <TouchableOpacity style={[styles.card, !item.leida && styles.cardUnread]} onPress={() => marcarLeida(item.id)} activeOpacity={0.8}>
...
function makeStyles(theme: ReturnType<typeof useTheme>) {
  return StyleSheet.create({
    ...
    cardUnread: { borderColor: theme.primaryBadge, backgroundColor: theme.primarySurface },
    ...
    cardMsg: { fontSize: 13, color: theme.textSecondary, marginTop: 3 },
    cardDate: { fontSize: 11, color: theme.textMuted, marginTop: 6 },
